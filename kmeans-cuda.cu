...
void kmeans(
  ...
  cudaMalloc((void **) &dev_centroids, num_centroids * num_coords * sizeof(double));

  cudaMemcpy(dev_points, points[0], num_points * num_coords * sizeof(double), cudaMemcpyHostToDevice);

  do {
    cudaMemcpy(dev_centroids, centroids[0], num_centroids * num_coords * sizeof(double), cudaMemcpyHostToDevice);
